...
First, I will load and tidy the data. The following code uses `mutate`
to update variables to appropriate types, convert reported_date into a
more readable format (YYY-MM-DD), and correct the capitalization of
victim_first and victim_last. It also creates two new variables: i) a
`city_state` variable that returns both city and state (e.g. “Baltimore,
MD”), and ii) a binary variable `solved` indicating whether the homicide
is solved (TRUE) or not (FALSE). Solved homicides are defined as
homicides for which the disposition is “Closed by arrest.” `filter` is
used to omit homicides in Dallas, TX; Phoenix, AZ; Kansas City, MO; and
Tulsa, AL, and to limit our analysis to homicides for which victim_race
is white or Black.
...
an R object. `broom::tidy` and `filter` are used to obtain the adjusted
odds ratio estimate and confidence interval for solving homicides
...
Now, I will run `glm` for each of the cities in the homicides_hw6
dataset, and extract the adjusted odds ratio and confidence interval for
solving homicides comparing male victims to female victims. The
following “tidy” pipeline uses `purrr::map`, `glm`, `broom::tidy`, list
columns, and `unnest` to create a dataframe with estimated ORs and CIs
for each city. `filter` and `select` are used to select our values of
interest.
...
create a plot showing the estimated odds ratios and confidence intervals
for each city. Cities are arranged in ascending order of estimated OR.
...
the adjusted OR \>= 1, which indicates that the odds of homicide
resolution for male victims is equal to or greater than the odds of
homicide resolution for female victims in these cities.
...
is 0, which is unexpected, because our menarche variable represents the
...
My selected model is bwt \~ babysex + bhead + blength + delwt +
fincome + gaweeks + , mheight + mrace + parity + ppwt + smoken This
model was selected using stepwise selection by AIC, a data-driven
model-building process. Stepwise selection is a combination of forward
and backwards model selection, during which
