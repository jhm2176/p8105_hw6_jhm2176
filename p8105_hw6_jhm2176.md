...
pipeline uses `purrr::map`, `glm`, `broom::tidy`, list columns, and
`unnest` to create a dataframe with estimated ORs and CIs for each city.
`filter` and `select` are used to select our values of interest.
