...
No missing data was identified. There is one observation where menarche is 0, which is unexpected, because our menarche variable represents the mother’s age at menarche (years). I will make note of this but refrain from changing the data.
...
tidy(fit_step)
```
My selected model is `r format(formula(fit_step))`  This model was selected using stepwise selection by AIC, a data-driven model-building process. Stepwise selection is a combination of forward and backwards model selection, during which 
