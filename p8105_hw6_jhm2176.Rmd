...
First, I will load and clean the data for regression analysis. `mutate` is used to convert babysex, frace, malform, and mrace from numeric variables to factor variables. `is.na` and `unique` are used to check for missing and unusual data. There is no missing data, but one observation where menarche is 0. Because our menarche variable represents the mother’s age at menarche (years), a result of 0 is unusual. 
